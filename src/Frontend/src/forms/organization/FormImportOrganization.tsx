...
import { ENUM_IMPORT_TYPES, ENUM_ORGANIZATIONS_IMPORT } from "@/constants/enums";
import { TFormImportProps } from "@/interfaces/component";
import { zodResolver } from "@hookform/resolvers/zod";
import { useFieldArray, useForm } from "react-hook-form";
import { z } from "zod";
import { sendImportOrganizationsDataGCS } from "@/services/organization";
...
const FormImportOrganization : React.FC<TFormImportProps> = ({ formData, setOpen }) => {

    const t = useTranslations("form.organization.import");
    ...
    const onSubmit = async (values: z.infer<typeof formData.schema>) => {
        ...
        const processor = await getProcessorImportSignedLink(ENUM_IMPORT_TYPES.organization, mime_type).catch(() => {
        ...
        try {
            await sendImportOrganizationsDataGCS(data)
            toast.success(tToast("success.form.import_organization"));
        } catch (error) {
            toast.error(tToast("errors.form.import_organization"));
        } finally {          
            setOpen(false);
                ...
                <section className="space-y-2">
                                        ...
                                        return (
                                            <FormItem className="space-y-2">
                                                ...
                                                >
                                                    ...
                                                    <SelectContent>
                                                        {ENUM_ORGANIZATIONS_IMPORT.map(option => (
                    ...
                    <div className="block">
                        <Button 
                            type="button" 
                            className="w-full" 
                            onClick={() => append({ type: ENUM_ORGANIZATIONS_IMPORT[0].value, data: '' })}
                        >
                            {t("add_key")}
                        </Button>
                    </div>
...
export default FormImportOrganization;