...
import { getShifts, getShiftById, deleteShift } from "@/services/shift";
import FormShift from "@/components/shifts/FormShift";
...
const Shifts : React.FC = () => {
    ...
    useEffect(() => {
        if (mounted) {
            queryClient.invalidateQueries({ queryKey: [CATEGORY_SHIFTS] });
        }
    ...
    const create = () => {
        setOptions({
            mode: MODE_CREATE,
            title: "form.shift.create.create_shift",
            subtitle: "form.shift.create.create_subtitle_shift",
            formData: {
                schema: SchemaShift,
    ...
    const editById = async (id: string) => {
        setOptions({
            mode: MODE_EDIT,
            title: "form.shift.edit.edit_shift",
            subtitle: "form.shift.edit.edit_subtitle_shift",
            formData: {
                schema: SchemaShift,
                defaultValues: () => getShiftById(id),
                category: CATEGORY_SHIFTS,
                id: id
            }
        });
    ...
    const deleteById = (id: string) => {
        setOptions({
            ...
            subtitle: "subtitle",
            formData: {
                confirm: async () => await deleteShift(id),
                category: CATEGORY_SHIFTS
            }
        });
    };
    ...
    return (
        <>
            <div className="sm:container pt-5 2xl:pt-10 mb-10">
                <h1 className="font-semibold text-2xl md:text-3xl text-primary dark:text-white mb-1">{t('form.shift.list_title')}</h1>
                <h2 className="text-black/80 dark:text-white/80 text-base md:text-xl">{t('form.shift.list_subtitle')}</h2>
            </div>
                    ...
                    <SheetContent className="overflow-auto w-full sm:min-w-[420px]">
                        ...
                        <div className="py-5">
                            {(options.mode === MODE_CREATE || options.mode === MODE_EDIT) && (
                                <FormShift mode={options.mode} formData={options.formData} setOpen={setOpenSheet} />
                            )}
                        </div>
                    </SheetContent>
                </Sheet>
