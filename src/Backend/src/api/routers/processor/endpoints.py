...
@router.post("/email/_handle", response_model=None)
async def send_email_handler(
    ...
    :return: status_code indicating success or failure.
    """
    logger.info("Starting processing email handler.")
    data = schemas.PubsubEmailData.parse_raw(
        base64.b64decode(body.message.data.decode("utf-8"))
    )
    logger.info("Start processing email data", extra={"data": str(data)})

    groups, _ = await list_groups(grade=data.grade, page_size=-1)
    users, _ = await list_users(groups=[g.id for g in groups], page_size=-1)
    to_emails = []
    for user in users:
        if not user.email_address:
            logger.warning(f"User {user.id} does not have an email_address")
        to_emails.append(user.email_address)
    ...
    email_subject = settings.get("email_subject", "Nova prova criada!")
    front_base_url = settings.get("front_base_url")

    current_dir = Path(__file__).resolve().parent
    html_file_path = current_dir / "email_template.html"
    with open(html_file_path, encoding="utf-8") as file:
        email_body = file.read()
        email_body = email_body.replace(
            'href="#"', f'href="{front_base_url}"'
    ...
    try:
        with smtplib.SMTP(smtp_server, smtp_port) as server:
            ...
            email_batches = chunk_list(to_emails, 5000)
            for batch in email_batches:
                ...
                server.sendmail(email_from, batch, message.as_string())
            server.quit()
